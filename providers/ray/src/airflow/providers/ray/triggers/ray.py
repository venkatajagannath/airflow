...
class RayJobTrigger(BaseTrigger):
    """
    Trigger that monitors the status of a Ray job.
    ...
    :param poll_interval: The interval in seconds at which to poll the job status. Defaults to 30.
    ...
    def hook(self) -> RayHook:
        """
        Lazily initialize and return a RayHook instance.

        :return: An instance of RayHook configured with the connection ID.
        """
        return RayHook(conn_id=self.conn_id)

    async def cleanup(self) -> None:
        """
        Clean up resources by deleting the Ray cluster.

        Called when the trigger encounters an exception, e.g. when a job is stopped through the
        UI after cluster startup and job submission.
        """
    ...
    async def _stream_logs(self) -> None:
        """Stream logs from the Ray job in real-time."""
    ...
    async def run(self) -> AsyncIterator[TriggerEvent]:
        """
        Asynchronously poll the Ray job status and yield events based on the job's state.
        ...
        self.log.info("Polling for job %s every %d seconds...", self.job_id, self.poll_interval)
