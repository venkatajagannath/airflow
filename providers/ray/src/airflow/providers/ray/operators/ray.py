...
from airflow.providers.ray.hooks.ray import TERMINAL_JOB_STATUSES, RayHook
...
class SetupRayCluster(BaseOperator):
    ...
    :param gpu_device_plugin_yaml: URL or path to the GPU device plugin YAML.
...
class SubmitRayJob(BaseOperator):
    ...
    This operator handles the submission of a Ray job to a Ray cluster and monitors its
    status until completion. It supports deferring execution and resuming based on job
    status changes, making it suitable for long-running jobs.
    ...
    :param ray_cluster_yaml: Path to the Ray cluster YAML configuration file.
    :param kuberay_version: Version of KubeRay to use. Defaults to "1.0.0".
    :param update_if_exists: Whether to update the Ray cluster if it already exists. Defaults to True.
    :param gpu_device_plugin_yaml: URL or path to the GPU device plugin YAML file.
    :param fetch_logs: Whether to fetch logs from the Ray job. Defaults to True.
    :param wait_for_completion: Whether to wait for the job to complete. Defaults to True.
    :param job_timeout_seconds: Maximum time to wait for job completion in seconds. Defaults to 600.
        Set to 0 if you want the job to run indefinitely without timeouts.
    :param poll_interval: Interval between job status checks in seconds. Defaults to 60.
    ...
    def execute_complete(self, context: Context, event: dict[str, Any]) -> None:
        ...
        :raises RayJobException: If the job execution fails, is cancelled, or reaches an unexpected state.
        """
        self.log.info("::endgroup::")
