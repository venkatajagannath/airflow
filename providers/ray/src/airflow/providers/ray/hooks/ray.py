...
class RayHook(KubernetesHook):  # type: ignore[misc]
    """
    ...
    def submit_ray_job(
        ...
        **job_config: Any,
        ...
        """
        ray_client = self.ray_client(dashboard_url=dashboard_url)
        job_id = ray_client.submit_job(entrypoint=entrypoint, runtime_env=runtime_env, **job_config)
    ...
    def delete_ray_job(self, dashboard_url: str | None, job_id: str) -> Any:
        ...
        return ray_client.delete_job(job_id=job_id)
    ...
    def get_ray_job_status(self, dashboard_url: str | None, job_id: str) -> JobStatus:
        ...
        """
        ray_client = self.ray_client(dashboard_url=dashboard_url)
        status = ray_client.get_job_status(job_id=job_id)
    ...
    def get_ray_job_logs(self, dashboard_url: str | None, job_id: str) -> str:
        ...
        """
        ray_client = self.ray_client(dashboard_url=dashboard_url)
        logs = ray_client.get_job_logs(job_id=job_id)
        return str(logs)

    async def get_ray_tail_logs(self, dashboard_url: str | None, job_id: str) -> AsyncIterator[str]:
        """
        Tail the logs of a submitted job asynchronously.
        ...
        """
        ray_client = self.ray_client(dashboard_url=dashboard_url)
        async for lines in ray_client.tail_job_logs(job_id):
            yield lines
    ...
    def _wait_for_load_balancer(
        ...
        for attempt in range(1, max_retries + 1):
            self.log.info("Attempt %d: Checking LoadBalancer status...", attempt)
    ...
    def _run_bash_command(
        ...
        try:
            result = subprocess.run(
                command, shell=True, check=True, text=True, capture_output=True, env=custom_env
            )
    ...
    def install_kuberay_operator(
        ...
        helm_command = (
            ...
            f"--version {version} --create-namespace --namespace {self.namespace} "
    ...
    def uninstall_kuberay_operator(self) -> tuple[str | None, str | None]:
        ...
        """
        helm_command = (
            f"helm uninstall kuberay-operator --namespace {self.namespace} --kubeconfig {self.kubeconfig}"
        )
